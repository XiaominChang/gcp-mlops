"""
Advertising ROI Model Training Script
Predicts revenue from advertising spend across 4 channels.
...
def read_campaign_data(file_path):
    """Read campaign spend data and extract year/month."""
...
def calculate_revenue_per_month(file_path):
    """Aggregate monthly revenue from raw revenue data."""
    df_revenue = pd.read_csv(file_path)
    df_revenue_per_month = df_revenue.groupby(
...
def calculate_spend_per_month(df):
    """Pivot campaign spend data to get per-channel monthly totals."""
    grouped_data = df.groupby(
        ["YEAR", "MONTH", "CHANNEL"], as_index=False
    )["TOTAL_COST"].sum()
...
def merge_dataframes(df_revenue, df_spend):
    """Merge revenue and spend DataFrames on year and month."""
...
def train_model(df):
    """Train a polynomial regression model with GridSearchCV."""
    numeric_features = ["SEARCH_ENGINE", "SOCIAL_MEDIA", "VIDEO", "EMAIL"]
    polynomial_features_degrees = 2

    numeric_transformer = Pipeline(steps=[
        ("poly", PolynomialFeatures(degree=polynomial_features_degrees)),
...
def evaluate_model(model, X_train, y_train, X_test, y_test):
    """Evaluate model using R-squared on train and test sets."""
...
def main():
    ...
    train_r2_score, test_r2_score = evaluate_model(
