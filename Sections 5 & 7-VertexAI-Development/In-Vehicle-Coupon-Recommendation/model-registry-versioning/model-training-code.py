"""
Coupon Recommendation - Stable Model Training (v1)
====================================================
Trains a baseline XGBoost classifier with fixed hyperparameters
and saves the artifact to GCS. This is the "stable" version.
...
def save_model_artifact(model):
    ...
    blob = bucket.blob(f"coupon-recommendation/artifacts/{artifact_name}")
    blob.upload_from_filename(artifact_name)
    print(f"Model saved to gs://{GCS_BUCKET}/coupon-recommendation/artifacts/{artifact_name}")
...
print("Training stable model (v1):")
...
model = train_model(x_sm_train, y_sm_train, max_depth=5, learning_rate=0.2, n_estimators=40)
evaluate_model(model, x_test_hashing, y_test, x_sm_train, y_sm_train)
